...
// Category Wise Products
        $homecategories = DB::table('home_category') 
            ->select('home_category.*') 
            ->orderBy('home_category.homecategory_id', 'ASC')->get();

          $count=1;
           $homecategory = array();
           foreach ($homecategories as $homecategori) { 
             $homecategory[]=$homecategori->homecategory_title;  
           }
           $result['homecategory'] = $homecategory;
        foreach ($homecategories as $homecategori) {
          $categorySlug = array('slug' => $homecategori->homecategory_slug);

            $cat_id = $this->products->getCategoryIdWithChild($categorySlug);
            $homecategorydata = array('page_number' => '0','type' => '', 'categories_id' => $cat_id, 'limit' => 10, 'min_price' => $min_price, 'max_price' => $max_price, 'is_feature' => true);
            
         $category = $this->products->getCategoriesByslug($categorySlug['slug']); 
         $home_category = $this->products->products($homecategorydata);
         
         if($category->count()) {
             $home_category['category'] = $category[0];
            $childs = DB::table('categories')
            ->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
            ->where('parent_id', $category[0]->categories_id)->limit(5)->get();
            // foreach($childs as $child) {
            //     $payload = array('page_number' => '0','type' => '', 'categories_id' => $child->categories_id, 'limit' => 10, 'min_price' => $min_price, 'max_price' => $max_price, 'is_feature' => true);
            //      $products = $this->products->products($payload);
            //      $child->products = $products;
            // }
              $result['children'][$count] = $childs;

         }
         
         $result['categorywiseproducts'][$count] = $home_category;
...
//current time
    ...
    public function getAjaxProducts($cat_slug){
        $categorySlug = array('slug' => $cat_slug);

        $cat_id = $this->products->getCategoryIdWithChild($categorySlug);
        $homecategorydata = array('page_number' => '0','type' => '', 'categories_id' => $cat_id, 'limit' => 10, 'min_price' => '', 'max_price' => '', 'is_feature' => true);
        $products = $this->products->products($homecategorydata);
        $itemsArray=[];
        // foreach($products['product_data'] as $product){
            $view= view('web.common.product_card_style.owl-column', ['product_data' => $products['product_data']]);
            // $itemsArray[]=$view;
        // }
        return $view;
