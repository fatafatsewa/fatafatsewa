...
class ProductsController extends Controller
{
    ...
    public function productImages($slug, Request $request)
    {
        $products = $this->products->getProductsBySlug($request->slug);

        $products_images = DB::table('products_images')
            ->LeftJoin('image_categories', 'products_images.image', '=', 'image_categories.image_id')
            ->select('image_categories.path as image_path', 'image_categories.image_type', 'products_images.color_id')
            ->where('products_id', '=', $products[0]->products_id)
            ->where('products_images.color_id', '=', $request->color_id)
            ->orderBy('sort_order', 'ASC')
            ->get();

        $view = view('web.details.product-images', ['product_images' => $products_images])->render();
        return $view;
    }
    ...
    public function applyEmiForm(Request $request)
    {
        ...
        if ($request->has('option_id')) {
            foreach ($request->option_id as $key => $opt_id) {
                $option = DB::table('products_options')->where('products_options_id', $opt_id)->first();
                $attr = DB::table();
                return $option->products_options_name;
            }
        }
        return $request->all();
    ...
    public function produtSearchSuggestion(Request $request)
    {
        if ($request->category !== '' && $request->category !== null) {
            $category = $this->categories->getCategoriesByslug($request->category);
            // return $category;
        }
        ...
        $products =  DB::table('products')
         ...
         $products =   $products->LeftJoin('products_to_categories', 'products.products_id', '=', 'products_to_categories.products_id')
                ->leftJoin('categories', 'categories.categories_id', '=', 'products_to_categories.categories_id')
                ->LeftJoin('categories_description', 'categories_description.categories_id', '=', 'products_to_categories.categories_id');

        if ($request->category !== '' && $request->category !== null) {
            $products =                $products->where('categories.categories_id', $category[0]->categories_id);
        }


        $products = $products->where('products_description.products_name', 'LIKE', '%' . $request->keyword . '%');
        return $products->get()->unique('products_name');
    }
}
