...
use App\Http\Helpers\EsewaHelper;
...
class OrdersController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        //DISABLING POINT USE ON BAR PRODUCTS
        $can_use_points = true;
        $top_cats = array_unique(collect($result['cart'])->pluck('categories')->flatten()->where('parent_id', 0)->pluck('categories_name')->toArray());
        if(count($top_cats) === 1) {
            if(in_array('Bar', $top_cats)) {
                $can_use_points = false;
            }
        }

        $result['can_use_points'] = $can_use_points;
    ...
    public function setPointsSession(Request $request)
    {

        if($request->total_price < 1000) {
            return redirect()->back()->with('error', 'Reward points cannot be used on order below Rs. 1000');
        }
        session()->put('use_reward_points', true);
        return redirect('/checkout');
    }
    ...
    public function checkout_shipping_address(Request $request)
    {
        ...
        session(['shipping_detail' => (object) $ship_detail]);
    ...
    public function getPaymentMethods()
    {
        ...
        //ESEWA
        $payments_setting = $this->order->payments_setting_for_esewa();

        if ($payments_setting['merchant_code']->environment == '0') {
            $esewa_environment = 'Test';
        } else {
            $esewa_environment = 'Live';
        }
        ...
        //CELLPAY
        $payments_setting = $this->order->payments_setting_for_cellpay();
    ...
    public function processEsewaPayment(Request $request)
    {
        ...
        $esewa_helper = new EsewaHelper($payments_setting['merchant_code']);
        $process_response = $esewa_helper->processPayment($request->amount);
        if (!$process_response) {
            return redirect()->back()->with('paymentResponse', 'Esewa payment failed. Please retry!');
        }
    }

    public function completeEsewaPayment(Request $request)
    {
        if ($request->status === 'failed') {
            return redirect()->back()->with(['paymentResponse' => 'error', 'paymentResponseData' => 'Esewa payment failed. Please retry!']);
        } else {
            $payments_setting = $this->order->payments_setting_for_esewa();

            $esewa_helper = new EsewaHelper($payments_setting['merchant_code']);

            $verification = $esewa_helper->verifyPayment($request->all());
            if ($verification) {
    ...
    public function processCellpayPayment(Request $request)
    {
        $status = $request->status;
        if ($status === 'cancel') {
            return redirect()->back()->with(['paymentResponse' => 'error', 'paymentResponseData' => 'Cellpay payment cancelled. Please retry!']);
        }

        if ($status === 'failed' || $status === 'Failed') {
            return redirect()->back()->with(['paymentResponse' => 'error', 'paymentResponseData' => 'Cellpay payment failed. Please retry!']);
        }
