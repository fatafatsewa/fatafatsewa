...
class AlertController extends Controller
{
	...
	public function emiAlert($data)
	{
		...
		if ($admin_email) {
			try {
				Mail::send('/mail/emis/emi-admin', ['data' => $data], function ($m) use ($data, $admin_email) {
					$m->to($admin_email)->subject("EMI request has been placed")->getSwiftMessage()
						->getHeaders()
						->addTextHeader('x-mailgun-native-send', 'true');
				});
			} catch (\Exception $e) {
		...
		if (!empty($data['emiData']['email'])) {
			try {
				Mail::send('/mail/emis/emi-customer', ['data' => $data], function ($m) use ($data) {
					$m->to($data['emiData']['email'])->subject("EMI request has been placed")->getSwiftMessage()
						->getHeaders()
						->addTextHeader('x-mailgun-native-send', 'true');

					if ($data['bank']['emi_form'] !== null && $data['bank']['emi_form'] !== '')
						$m->attach('resources/assets/banks/' . $data['bank']['id'] . '/' . $data['bank']['emi_form']);
				});
			} catch (\Exception $e) {
				return view('errors.not_install');
			}
		}
		if (!empty($data['bank']['email'])) {
			try {
				Mail::send('/mail/emis/emi-bank', ['data' => $data], function ($m) use ($data) {
					$m->to($data['bank']['email'])->subject("EMI request has been placed")->getSwiftMessage()
						->getHeaders()
						->addTextHeader('x-mailgun-native-send', 'true');
					if ($data['bank']['emi_form'] !== null && $data['bank']['emi_form'] !== '')
						$m->attach('resources/assets/banks/' . $data['bank']['id'] . '/' . $data['bank']['emi_form']);
