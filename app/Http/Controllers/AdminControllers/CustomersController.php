...
class CustomersController extends Controller
{
    ...
    public function viewDetail(Request $request)
    {
        ...
        return view("admin.customers.detail", $title)->with('data', $customerData)->with('result', $result)->with('allimage', $allimage);
    }
    ...
    public function update(Request $request)
    {
        ...
        //    dd((int)$request->reward_points, $user_data->reward_points);

        if ((int)$request->reward_points !== $user_data->reward_points) {
            if ($request->reward_points > $user_data->reward_points) {
                $message = 'increased';
                $amount = $request->reward_points - $user_data->reward_points;
            } else {
                $message = 'reduced';
                $amount = $user_data->reward_points - $request->reward_points;
            }

            //REWARD LOG
            RewardPointLog::create([
                'user_id' => $user_data->id,
                'action' => ($message == 'increased') ? '+':'-',
                'points' => $amount,
                'remarks' => $amount . ' points '.$message.' (Points '. $message.' by admin.)'
            ]);
        }
