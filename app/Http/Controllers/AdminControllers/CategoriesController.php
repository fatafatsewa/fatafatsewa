...
class CategoriesController extends Controller
{
  ...
  public function insert(Request $request)
  {
    ...
    $categories_id = $this->Categories->insert($uploadImage, $date_added, $parent_id, $uploadIcon, $categories_status, $meta);
    $slug_flag = false;
  ...
  public function update(Request $request)
  {
    ...
    $custom_dashboard = $request->custom_dashboard;
    //get function from other controller
    $languages = $this->varseting->getLanguages();
    ...
    $meta['description'] = $request->meta_description;
    $meta['keywords'] = $request->meta_keywords;
    $updateCategory = $this->Categories->updaterecord($categories_id, $uploadImage, $uploadIcon, $last_modified, $parent_id, $slug, $categories_status, $custom_dashboard, $meta);
  ...
  //CUSTOM DASH
  public function getBuildDashboard(Request $request)
  {
    $title = array('pageTitle' => Lang::get("labels.EditCategories"));
    ...
    $category = $this->Categories->getCategoryById($request->id);

    $banners = CategoryBanner::where('category_id', $request->id)->get();
    $result['banners'] = $banners;

    $childs = DB::table('categories')
      ->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
      ->where('parent_id', $category[0]->categories_id)->get();
    $result['category'] = $category;
    $result['childs'] = $childs;

    $dashboard_categories = $this->Categories->getDashboardCategories($request->id);
    $result['dashboard_categories'] = $dashboard_categories;
    return view("admin.categories.build-dashboard", $title)->with('result', $result);
  }

  public function addCategoryBanner($category_id, Request $request)
  {
    $this->validate($request, [
      'banner_image' => 'required|mimes:jpg,png,jpeg,JPG'
    ]);

    $file = $this->moveFile($request->file('banner_image'), $category_id);
  ...
  public function moveFile($file, $category_id)
  {
    $fileName = Carbon::now()->timestamp . '-' . dechex(rand(1000, 100000)) . '.' . $file->getClientOriginalExtension();
    $path = $file->move(public_path('images/category-banners/' . $category_id), $fileName);
    return url('/images/category-banners/' . $category_id . '/' . $fileName);
  }

  public function deleteCategoryBanner($category_id, $banner_id)
  {
    CategoryBanner::where(['category_id' => $category_id, 'id' => $banner_id])->delete();
    return redirect()->back()->with('message', "Banner image deleted successfully");
  }

  public function addDashboardCategory($category_id, Request $request)
  {
    $category = DB::table('categories')->where(['categories_id' => $category_id])->first();
    $existing_sub_categories = $category->dashboard_sub_categories;
    $sub_cats_array = explode(',', $existing_sub_categories);

    if(in_array($request->category_id, $sub_cats_array)) {
      return redirect()->back()->with('error', "Category already exists");
    }
    $sub_cats_array[] = $request->category_id;
    $new_cats = implode(',', $sub_cats_array);

    DB::table('categories')->where(['categories_id' => $category_id])->update([
      'dashboard_sub_categories' => $new_cats
    ]);
    return redirect()->back()->with('message', "Category added successfully");
  }

  public function removeDashboardCategory($id, $cat_id) {
    $category = DB::table('categories')->where(['categories_id' => $id])->first();
    $existing_sub_categories = $category->dashboard_sub_categories;
    $sub_cats_array = explode(',', $existing_sub_categories);
    $new_arr = array_diff($sub_cats_array,[$cat_id]);
    $new_cats = implode(',', $new_arr);

    DB::table('categories')->where(['categories_id' => $id])->update([
      'dashboard_sub_categories' => $new_cats
    ]);
