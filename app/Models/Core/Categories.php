...
class Categories extends Model
{
    ...
    public function paginator(){
      ...
      $categories = Categories::sortable(['categories_id'=>'ASC'])
           ...
           ->LeftJoin('image_categories as categoryTable', function ($join) {
            ...
            'categories.updated_at as last_modified', 'categories_description.categories_name as name', 'categories.custom_dashboard as custom_dashboard',
    ...
    public function insert($uploadImage,$date_added,$parent_id,$uploadIcon,$categories_status, $meta){
    ...
    public function updaterecord($categories_id,$uploadImage,$uploadIcon,$last_modified,$parent_id,$slug,$categories_status, $custom_dashboard, $meta){
    ...
    public function editsubcategory($request){
        $editSubCategory = DB::table('categories')
            ->leftJoin('image_categories as categoryTable','categoryTable.image_id', '=', 'categories.categories_image')
            ->leftJoin('image_categories as iconTable','iconTable.image_id', '=', 'categories.categories_icon')
            ->select('categories.categories_id as id', 'categories.meta_title as meta_title', 'categories.meta_description as meta_description', 'categories.meta_keywords as meta_keywords', 'categories.custom_dashboard as custom_dashboard' , 'categories.categories_image as image', 'categories.categories_icon as icon',  'categories.created_at as date_added', 'categories.updated_at as last_modified',
    ...
    public function getCategoriesByslug($slug)
    {
        ...
        ->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
        //->where('categories.categories_status', 1)
        ->where('categories_slug', $slug)
        ->get();
        return $category;
    }

    public function getDashboardCategories($category_id){
        $categories = DB::table('categories')->where('categories_id', $category_id)->select('dashboard_sub_categories')->first();
        $dashboard_sub_categories = explode(',', $categories->dashboard_sub_categories);

        $sub_categories = DB::table('categories')
        ->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
        ->whereIn('categories.categories_id', $dashboard_sub_categories)->get();
