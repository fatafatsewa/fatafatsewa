...
class Customer extends Model
{
    ...
    public function handleSocialLoginCallback($social)
    {
        ...
        if (auth()->guard('customer')->loginUsingId($customers_id, true)) {
    ...
    public function signupProcess($request, $referred_by = null)
    {
        $res = array();
        $old_session = Session::getId();
        ...
        $data = array(
            ...
            'user_name' => filter_var($request->email, FILTER_VALIDATE_EMAIL) ? null : $request->email,
            'password' => Hash::make($password),
            'created_at' => $date,
            'updated_at' => $date,
            ...
            if (DB::table('users')->insert([
                ...
                if (auth()->guard('customer')->attempt($request->only($login_type, 'password'))) {
                    ...
                    RewardPointLog::create([
                        ...
                        'action' => '+',
                        'points' => 500,
                        'remarks' => '500 points added (Account Registered)'
                    ]);

                    if (!$customer->hasVerifiedEmail()) {
                        $customer->sendEmailVerificationNotification();
                        $res['auth'] = "false";
                        $res['verfication'] = "true";
                        return $res;
                    }
                    //set session
                    session(['customers_id' => $customer->id]);
