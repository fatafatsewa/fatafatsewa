...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        $points_used = 0;
        if (session('use_reward_points') && session('usable_points'))
            $points_used = session('usable_points');
        ...
        if ($payment_method == 'braintree') {
            ...
            $esewa = $this->payments_setting_for_esewa();
            $payment_method_name = $esewa['merchant_code']->name;
            $payment_status = 'success';
            $order_information = ['oid' => $request->oid, 'refId' => $request->refId];
        } else if ($payment_method == 'cellpay') {
            $cellpay = $this->payments_setting_for_cellpay();
            $payment_method_name = $cellpay['merchant_code']->name;
            $payment_status = 'success';
            $order_information = ['invoice_no' => $request->invoice_no, 'refId' => $request->refId];
        }
        ...
        if ($payment_status == 'success') {
            ...
            session()->forget('usable_points');
            session()->forget('use_reward_points');
            ...
            if ($points_used) {
                ...
                DB::table('users')->where('id', $customer->id)->update([
                    'reward_points' => $customer->reward_points - $points_used
                ]);
                ...
                RewardPointLog::create([
                    ...
                    'action' => '-',
                    'points' => $points_used,
                    'remarks' => $points_used . ' points deducted (Used on order id ' . $orders_id . ')'
                ]);
            }
    ...
    public function payments_setting_for_esewa()
    {
        $payments_setting = DB::table('payment_methods_detail')
            ->leftjoin('payment_description', 'payment_description.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
            ->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
    ...
    public function payments_setting_for_cellpay()
    {
        $payments_setting = DB::table('payment_methods_detail')
            ->leftjoin('payment_description', 'payment_description.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
            ->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
