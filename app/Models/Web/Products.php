...
public function getCategoriesId($data)
    {
        $category = DB::table('categories')->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')->where('categories_slug', $data['slug'])->get();
           
                if ($category->isNotEmpty()) {
                    $cat_id = $category[0]->categories_id;
                    return $cat_id;
                }
                else
                {
    ...
    public function getCategoryIdWithChild($data)
    {
        $cat_ids = [];
        $category = DB::table('categories')->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')->where('categories_slug', $data['slug'])->get();
                if ($category->isNotEmpty()) {
                    ...
                    $childs = DB::table('categories')->where('parent_id', $cat_id)->pluck('categories_id')->toArray();
    ...
    public function products($data)
    {
        ...
        if(isset($data['emi_enabled']) && $data['emi_enabled'] === true) {
            $categories->where('products.emi_enabled', '=', 1);
        }
        ...
        if (!empty($data['categories_id'])) {
            if(is_array($data['categories_id'])) {
                $categories->whereIn('products_to_categories.categories_id', $data['categories_id']);
