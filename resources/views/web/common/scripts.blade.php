...
<script src="{!! asset('web/custom/assets/themes/fatafatsewa/vendor/owl-carousel/owl.carousel.min.js') !!}"></script>
<script src="{{ asset('web/js/jquery-ui.js') }}"> </script>

<script src="{!! asset('web/custom/assets/js/custom.js') !!}"></script>
...
jQuery(document).on('click','#allow-cookies', function(e){
  ...
  function cartPrice(){
    var i = 0;
    $("input:radio.currentstock:checked").each(function() {
      var value_price = $(this).attr('value_price');
      var attributes_value = $(this).attr('attributes_value');
...
jQuery(document).on('click', '.apply--emi', function(e) {
  var formData = jQuery("#add-Product-form").serializeArray();

  jQuery.ajax({
    url: '/product/emi-application',
    type: "POST",
    data: formData,
   headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
    success: function(res) {
...
jQuery(document).on('click', '.product-buy-now', function(e){
  var formData = jQuery("#add-Product-form").serializeArray();
...
jQuery(document).ready(function() {
  ...
  jQuery(document).on('change', '.{{ $functionValue }}', function(e){
      //changing active status
        $(`.attribute--${$(this).attr('name')}`).removeClass('active');
...
function paymentMethods(){
  ...
  //jQuery(document).on('click', '#midtrans_button', function(e){
    ...
    if(payment_method === 'esewa') {
			jQuery("#esewa_payment_section").show();
		}else {
			jQuery("#esewa_payment_section").hide();
		}

    if(payment_method === 'cellpay') {
			jQuery(".cellpay_payment_section").show();
		}else {
			jQuery(".cellpay_payment_section").hide();
		}
...
function getQuantity(){
  ...
  $("input:radio.currentstock:checked").each(function() {
    ...
    selected_attrs.push($(this).attr('attribute_name')) 
    jQuery('#function_' + i).val(value_price);
    jQuery('#attributeids_' + i++).val(attributes_value);

    if($(this).attr('option_name').toLowerCase() === 'color') {
      
      $.ajax({
        type:"GET",
        url: `/ajax/product-images/${$(this).attr('product_slug')}`,
        data:{color_id: $(this).attr('attributes_value')},
        success:function(data){
          $('.slider-for').slick('unslick');
          $('.slider-nav').slick('unslick');

          $('.product--detail__images').empty().html(data)
          setTimeout(function() {
  ...
  jQuery('.ex1').zoom({
    touch: false
          ...
          // let attr_val = $(this).attr('attributes_value');
          // if($('#product--image-'+attr_val)[0])
          //   $('#product--image-'+attr_val).trigger('click');
        }
  })

  $(".product--attributes").html(selected_attrs.join(', '))
  // jQuery(".currentstock").each(function() {
  //   var value_price = jQuery('option:selected', this).attr('value_price');
  //   var attributes_value = jQuery('option:selected', this).attr('attributes_value');
  //   jQuery('#function_' + i).val(value_price);
...
$(document).ready(function() {
  $(document).on('click', '.trigger--login', function() {
    ...
    $('#requestOTP').modal('hide')
    $('#loginModal').modal('show')
  })
  $(document).on('click', '.trigger--register', function() {
    $('#loginModal').modal('hide')
    $('#requestOTP').modal('hide')
    $('#registerModal').modal('show')
  })

  $(document).on('click', '.trigger--otp-request', function() {
    $('#loginModal').modal('hide')
    $('#registerModal').modal('hide')
    $('#requestOTP').modal('show')
  })

  $(document).on('submit', '.website--login-form', function(event) {
      event.preventDefault();

      let context = $(this)
      let payload = $(this).serialize();
      let btn = $(this).find('.login--button')
      ...
      $.ajax({
        ...
        if(data.success) {
          ...
          btn.html("Log In");
          if(data.message)
            context.find('.error--area').html('<div class="alert alert-danger">'+ data.message +'</div>')
          else
            context.find('.error--area').html('<div class="alert alert-danger">You are not allowed to login with these credentials </div>')
        }
      })
  })


  $(document).on('submit', '.website--register-form', function(event) {
      event.preventDefault();

      let context = $(this)
      let payload = $(this).serialize(); 
      let btn = $(this).find('.register-button')
      btn.prop('disabled', true);
      $(this).find('.help-block').empty()
      btn.html("Creating Account")
      $.ajax({
        url: '/ajaxsignupProcess',
        type: 'POST',
        data: payload,
        error: function(error) {
          btn.prop('disabled', false)
          btn.html("CREATE AN ACCOUNT");
         for(error_ in error.responseJSON.errors) {
           context.find(`.error-${error_}`).empty().html(error.responseJSON.errors[error_][0])
         }
        }
      }).done(function(data) {
        if(data.success) {
          ...
          if(data.message)
            ...
            context.find('.error--area').html('<div class="alert alert-danger">Something went wrong. Please Retry!!</div>')
        }
      })
  })

  $('#form-mobile--login').submit(function(event) {
      event.preventDefault()
      let context = $(this);
      var phone_number = $('#mobile-login-phone').val();
      let button = $(this).find('.button-form-submit')
      if (phone_number && !isNaN(phone_number)) {
        button.html("Sending OTP...");
    ...
    $('#form-validate--otp').submit(function(event) {
      event.preventDefault();

      var otp_code = $('#otp-code').val();
      var button = $(this).find('.validate-otp-button');
      button.html('Validating OTP...')
      ...
      $.ajax({
          ...
          success: function(data) {
            ...
            $.ajax({
              url: '/otp-login-process',
              method: "POST",
              data: {
                phone_number : $('#mobile-login-phone').val(),
                _token: '{{ csrf_token() }}'
              },
            ...
            button.prop('disabled', false)
            button.html("Validate OTP");
            $('.otp-error-section').html('Incorrect OTP. Please retry!!');
          }
          },
          error:function() {
            button.prop('disabled', false)
            button.html("Validate OTP");
            $('.otp-error-section').html('Something went wrong. Please retry!!');
          }
        }) 
    })
})
