@php
$desc = str_replace('\"', '', $result['detail']['product_data'][0]->products_description); //removing rowspan
$desc = str_replace("<table", '<table class="table"', $desc);
@endphp
@section('header-meta')
<title>{{ $result['detail']['product_data'][0]->products_name }} | Fatafatsewa</title>
<meta name="description" content="Buy {{ $result['detail']['product_data'][0]->products_name }} online at best price in Nepal - Fatafatsewa"/>

  <meta property="og:title" content="{{ $result['detail']['product_data'][0]->products_name }} - Fatafatsewa" >
    <meta property="og:url" content="{{ request()->url() }}" >
    <meta property="og:description" content="Buy {{ $result['detail']['product_data'][0]->products_name }} online at best price in Nepal - Fatafatsewa" >
    <meta name="twitter:title" content="{{ $result['detail']['product_data'][0]->products_name }} - Fatafatsewa" >
    <meta name="twitter:description" content="Buy {{ $result['detail']['product_data'][0]->products_name }} online at best price in Nepal - Fatafatsewa" >
...
@endsection
<div class="container-fuild breadcrumb-bg" >
...
@if($result['detail']['product_data'])
  <div class="container">
    <div class="page-heading-title">
        <h2> {{$result['detail']['product_data'][0]->products_name}} <span class="product--attributes"></span>
        </h2>         
    </div>
</div>
...
<section class="product-page">
  <div class="container"> 
    @if(session('emi_applied') && session('emi_applied') == true)
      <div class="alert alert-success">
        Thank you for applying EMI. We will get back to you soon.  
      </div>
      <div class="text-center mb-3">
      <a href="{{ session('emi_data')['bank']->emi_form_file }}" download class="btn btn-secondary">Download Emi Form</a>
      </div>
    @endif
    ...
    <div class="row">
      <div class="col-12 col-lg-4  ">
          ...
          <div class="product--detail__images">
           @include('web.details.product-images', ['product_images' => $result['detail']['product_data'][0]->images, 'default_image' => $result['detail']['product_data'][0]->default_images])
          </div>
           @show
      </div>
        ...
        <div class="row">
            <div class="col-12 col-md-12">
              ...
              @endif
                ...
                <h5 class="pro-title">{{$result['detail']['product_data'][0]->products_name}} <span class="product--attributes"></span></h5>
          ...
          <div class="price">                     
            ...
            ?>
            @if(!empty($result['detail']['product_data'][0]->flash_price))
            <sub class="total_price currencyField">{{Session::get('symbol_left')}} {{$flash_price+0}}{{Session::get('symbol_right')}}</sub>
            ...
            <price class="total_price currencyField">{{Session::get('symbol_left')}} {{$discount_price+0}}{{Session::get('symbol_right')}}</price>
            <span>{{Session::get('symbol_left')}} {{$orignal_price+0}}{{Session::get('symbol_right')}} </span> 
            @else
            <price class="total_price currencyField">{{Session::get('symbol_left')}} {{$orignal_price+0}}{{Session::get('symbol_right')}}</price>
            @endif
            </div>
          ...
          <form name="attributes" id="add-Product-form" method="post" >
            ...
            <input type="hidden" id="max_order" value="@if(!empty($result['detail']['product_data'][0]->products_max_stock)) {{ $result['detail']['product_data'][0]->products_max_stock }} @else 0 @endif" >
             ...
             ?>
               ...
               <input id="attributeids_<?=$index?>" type="hidden" name="attributeid[]" value="" >
                 ...
                 <div class="attributes col-12 box">
                     ...
                     @endif --}}
                     <label class="attribute-selectors attribute--{{ $attributes_data['option']['id'] }} {{ ($key===0) ? 'active' :'' }}">
                      <input type="radio" option_name="{{ $attributes_data['option']['name'] }}" name="{{ $attributes_data['option']['id'] }}" id="attribute_{{ $values_data['id'] }}" onChange="getQuantity()" value="{{ $values_data['id'] }}"  value_price ="{{ $values_data['price']+0 }}" class="currentstock attributeid_<?=$index?> d-none" prefix = '{{ $values_data['price_prefix'] }}' attributes_value="{{ $values_data['products_attributes_id'] }}" @if($key===0) checked @endif attribute_name="{{ $values_data['value'] }}" product_slug={{ $result['detail']['product_data'][0]->products_slug }} }}>
                      {{ $values_data['value'] }}
                    </label>
                    @endforeach
                    @php 
             ...
             @if(!empty($result['detail']['product_data'][0]->available_offers))
             <div class="available-offers offers--limit">
              <strong for="">Available Offers</strong>
              <p>{!! $result['detail']['product_data'][0]->available_offers !!}
                
              </p>
            </div>

            <div class="available-offers offers--full d-none">
              <strong for="">Available Offers</strong>
              <p>{!! $result['detail']['product_data'][0]->available_offers !!}
              </p>
              <strong class="offer--link offers--see-less">See less</strong>
            </div>
            @endif

            @if($result['detail']['product_data'][0]->highlight_specifications)
            @php 
            $specs = explode('|', $result['detail']['product_data'][0]->highlight_specifications);
            @endphp

            <strong>Highlights</strong>
            <ul class="mt-2" style="columns: 2">
          ...
          <div class="pro-counter" @if(!empty($result['detail']['product_data'][0]->flash_start_date) and $result['detail']['product_data'][0]->server_time < $result['detail']['product_data'][0]->flash_start_date ) style="display: none" @endif>
              <div class="input-group item-quantity">                    
                ...
                @if(!empty($result['detail']['product_data'][0]->flash_start_date) and $result['detail']['product_data'][0]->server_time < $result['detail']['product_data'][0]->flash_start_date )
                  ...
                  <div class="web-shop--buttons">
                    @if($result['detail']['product_data'][0]->products_type == 0)
                        @if($result['detail']['product_data'][0]->defaultStock == 0) 
                          <button class="btn btn-lg swipe-to-top mb-2 btn-danger " type="button">@lang('website.Out of Stock')</button>
                        @else
                            ...
                            <button class="btn btn-secondary btn-lg swipe-to-top product-buy-now"  type="button" products_id="{{$result['detail']['product_data'][0]->products_id}}">Buy Now</button>

                            @if($result['detail']['product_data'][0]->emi_enabled)
                            <button type="button" class="btn btn-secondary btn-lg btn--apply--emi">Apply EMI</button>
                            @endif
                        @endif
                    @else
                          ...
                          <button class="btn btn-danger btn btn-lg mb-2 swipe-to-top  stock-out-cart" hidden type="button">@lang('website.Out of Stock')</button>
                          <button class="btn btn-secondary btn-lg swipe-to-top product-buy-now"  type="button" products_id="{{$result['detail']['product_data'][0]->products_id}}">Buy Now</button>
                          @if($result['detail']['product_data'][0]->emi_enabled)
                          <button type="button" class="btn btn-secondary btn-lg btn--apply--emi">Apply EMI</button>
                            @endif
                  ...
                  <article>
                    <div class="thumb">
                      <div class="badges">
                        ...
                        <a href="{{ route('website.product-detail', $products->products_slug) }}">
    ...
    {{-- DESCIRPTION TABS --}}
      <div class="col-12 col-md-12 mt-4">
        <div class="nav nav-pills" role="tablist">
          <a class="nav-link active nav-item" href="#description" id="description-tab" data-toggle="pill" role="tab">@lang('website.Descriptions')</a>
          @if($result['detail']['product_data'][0]->products_video_link) 
          <a class="nav-link nav-item" href="#product_video" id="video-tab" data-toggle="pill" role="tab" >Video</a>
          @endif
          @if($result['detail']['product_data'][0]->warrenty)
            <a class="nav-link nav-item" href="#warranty" id="review-tab" data-toggle="pill" role="tab" >Warranty</a>
          @endif
          @if($result['detail']['product_data'][0]->emi_enabled)
           <a class="nav-link nav-item tab--apply-emi" href="#product_emi_application" id="emi-tab" data-toggle="pill" role="tab" >Apply EMI</a>
          @endif
        </div> 
        <div class="tab-content">
          <div role="tabpanel" class="tab-pane fade active show table-responsive" id="description" aria-labelledby="description-tab">
            <div id="" class="specs-list div-faded">
            {!! $desc !!}    
          </div>    
          <div class="mt-5">
            <p class="expand--description" style="cursor: pointer">Read more specs..</p>
          </div>
          </div>  

          @if($result['detail']['product_data'][0]->products_video_link) 
          <div role="tabpanel" class="tab-pane fade " id="product_video" aria-labelledby="video-tab">
          ...
          @if($result['detail']['product_data'][0]->warrenty)
          <div role="tabpanel" class="tab-pane fade" id="warranty" aria-labelledby="warranty-tab">
            {!! $result['detail']['product_data'][0]->warrenty !!}              
          </div>  
          @endif

          @if($result['detail']['product_data'][0]->emi_enabled)
          
          <div role="tabpanel" class="tab-pane fade  " id="product_emi_application" aria-labelledby="emi-tab">
            @include('web.details.emi-application')
          </div>
          @endif
      </div>
...
<style>
  ...
  .web-shop--buttons .btn--apply--emi{
    width: 97%;
  ...
  <script>
    ...
    jQuery(document).ready(function(e) {
      $(document).on('click', '.btn--apply--emi', function() {
        $('.tab--apply-emi').trigger('click')

        $('html, body').animate({
        scrollTop: $(".tab--apply-emi").offset().top
    }, 2000);
      })
      ...
      @if(!empty($result['detail']['product_data'][0]->flash_start_date))
        ...
        var x_{{$result['detail']['product_data'][0]->products_id}} = setInterval(function() {
          ...
          //documen      this.``
t.getElementById(counter_id_{{$result['detail']['product_data'][0]->products_id}}).innerHTML = "EXPIRED";
    ...
    <script>
      ...
      $(document).on('click', '.expand--description', function() {
        $('.specs-list').css('max-height', '100000px')
        $('.specs-list').removeClass('div-faded')

        $(this).remove();
      })
      $(document).on('click', '.offers--see-more', function() {
        $('.offers--limit').addClass('d-none')
        $('.offers--full').removeClass('d-none')

      })

      $(document).on('click', '.offers--see-less', function() {
        $('.offers--limit').removeClass('d-none')
        $('.offers--full').addClass('d-none')

      })
      
    $(document).on('click', '.btn--submit-emi-apply', async function() {
      $('#error-wrapper').html("")
      $(this).prop('disabled', true);
      $(this).html("Applying EMI...");
      let payload = {
        'product_id': $('#emi-product-id').val(),
        'full_name': $('#emi-full-name').val() ,
        'email' : $('#emi-email').val(),
        'bank': $('#emi-bank').val(),
        'emi_type': $('#emi-type').val()
      }
     

      try
      {
        let response = await axios.post('/product/apply-emi', payload);
            if(response.data.success === true) {
              $('.emi--modal--body').html(`<h3>${response.data.message}</h3>`)
            }
            $(this).hide();
      } catch(error) {
        ...
        for(error_ in errors){
          error_text = error_text  + "<br>" + errors[error_]
        }
        $('#error-wrapper').html(`<div class='alert alert-danger'>${error_text}</div>`)
